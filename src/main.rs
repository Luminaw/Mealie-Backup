...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    backup_service
        .create_backup(Some("en-US".to_string()))
        .await?;

    // Retrieve all backups and get the first one
    let all_backups = backup_service
        .get_all_backups(Some("en-US".to_string()))
        .await?;
    ...
    let backup_token = backup_service
        .get_backup(&backup_name.clone(), Some("en-US".to_string()))
        .await?;
    let backup_data = backup_service.download_backup(backup_token).await?;

    save_backup(backup_name, &set_config.local_backups_location, backup_data)
        .await
        .expect("Failed to save backup");

    info!("Backup downloaded and saved successfully");

    cleanup_old_backups(&all_backups, &backup_service, set_config.max_server_backups)
        .await
        .expect("Failed to cleanup old backups");
    cleanup_old_local_backups(
        &set_config.local_backups_location,
        set_config.max_local_backups,
    )
    .await?;
...
async fn save_backup(
    backup_name: &str,
    local_backups_location: &str,
    backup_data: Vec<u8>,
) -> Result<(), Box<dyn std::error::Error>> {
    let mut file =
        tokio::fs::File::create(format!("{}/{}", local_backups_location, backup_name)).await?;
    ...
    if all_backups.backups.len() >= max_server_backups {
        ...
        if let Some(oldest_backup) = all_backups.backups.iter().min_by_key(|b| {
            DateTime::parse_from_rfc3339(&b.date)
                .unwrap_or_else(|_| DateTime::<Utc>::MIN_UTC.into())
        }) {
            // Delete the oldest backup
            backup_service
                .delete_backup(oldest_backup.name.clone(), Some("en-US".to_string()))
                .await?;
...
async fn cleanup_old_local_backups(
    local_backups_location: &str,
    max_local_backups: usize,
) -> Result<(), Box<dyn std::error::Error>> {
